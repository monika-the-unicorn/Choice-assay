...
class TestChoiceAssaySensorWithLEDs(unittest.TestCase):
    ...
    def test_dual_arena_motion_detection_logic(self):
        ...
        with patch("choice_assay.my_choice_assay_sensor.cv2") as mock_cv2:
            ...
            mock_cv2.absdiff.side_effect = lambda img1, img2: np.abs(
                img1.astype(int) - img2.astype(int)
            ).astype(np.uint8)
            mock_cv2.threshold.side_effect = lambda img, thresh, maxval, type: (
                None,
    ...
    def test_sensor_initialization_without_gpio_errors(self, mock_GPIO, mock_cv2, mock_Picamera2):
        ...
        with patch("RPi.GPIO.setmode", side_effect=Exception("No GPIO available")):
            ...
            try:
                _ = ChoiceAssaySensorWithLEDs(self.config)
...
class TestChoiceAssaySensorCfg(unittest.TestCase):
    ...
    def test_configuration_modification(self):
        ...
        config = replace(
            DEFAULT_CA_SENSOR_CFG, frame_rate=10, motion_threshold=500, left_detection_roi=(10, 10, 300, 200)
        )
