...
def create_synthetic_frame(
    width=1640, height=1232, add_left_motion=False, add_right_motion=False, motion_intensity=50
):
    ...
    if add_left_motion:
        ...
        noise = np.random.randint(
            -motion_intensity, motion_intensity, frame[y1:y2, x1:x2].shape, dtype=np.int16
        )
...
def test_configuration():
    ...
    print(
        "✓ Custom config: "
        f"threshold={custom_config.motion_threshold}, "
        f"grace={custom_config.grace_period_seconds}"
    )
...
def test_motion_detection():
    ...
    with (
        ...
        frame_both_motion = create_synthetic_frame(
            add_left_motion=True, add_right_motion=True, motion_intensity=100
        )
...
def test_mutual_exclusivity():
    ...
    with (
        ...
        test_config = replace(DEFAULT_CA_SENSOR_CFG, detection_frames_needed=3, grace_period_seconds=1.0)
        ...
        for i in range(5):
            ...
            print(
                "    Counters - "
                f"left: {sensor.left_detection_counter}, "
                f"right: {sensor.right_detection_counter}"
            )
...
def test_state_management():
    ...
    with (
        ...
        print(
            "✓ Initial state: "
            f"left_motion={sensor.left_motion_detected}, "
            f"right_motion={sensor.right_motion_detected}"
        )
        print(
            "✓ Initial counters: "
            f"left={sensor.left_detection_counter}, "
