...
def get_mp4_NameWithDate(nameIn=".mp4"):
    ...
    filename = nameIn.replace(".mp4","_{0}.mp4").format(datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S"))
...
newpath = "/home/RPi9/Documents/Beecam_RPi9_" + datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S") ###EDIT THIS BEFORE RUNNING PROGRAM
...
def update_log_file_start_L():
    with open(LOG_FILE_NAME, "a") as f:
        ...
        f.write("Video recording in left arena started at " + datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S"))
        f.write("\n")
def update_log_file_end_L():
    with open(LOG_FILE_NAME, "a") as f:
        #Opens the file and adds a new line to it
        f.write("Video recording in left arena ended at " + datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S"))
        f.write("\n")
def update_log_file_start_R():
    with open(LOG_FILE_NAME, "a") as f:
        #Opens the file and adds a new line to it
        f.write("Video recording in right arena started at " + datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S"))
        f.write("\n")
def update_log_file_end_R():
    with open(LOG_FILE_NAME, "a") as f:
        #Opens the file and adds a new line to it
        f.write("Video recording in right arena ended at " + datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S"))
        f.write("\n")
...
#    cv2.imshow("live video", new_frame) #Change to 'threshold' on later line to see black and white vector changes; 'new_frame' to see unedited image
    if frame_counter == 1:
        ...
        difference_L = cv2.absdiff(old_frame_bwgb[y1_det:y2_det, x1_L_det:x2_L_det], new_frame_bwgb[y1_det:y2_det, x1_L_det:x2_L_det]) #Calculate difference bw old and new frames in left ROI
        difference_R = cv2.absdiff(old_frame_bwgb[y1_det:y2_det, x1_R_det:x2_R_det], new_frame_bwgb[y1_det:y2_det, x1_R_det:x2_R_det]) #Do the same for the right ROI
        threshold_L = cv2.threshold(difference_L, 3, 255, cv2.THRESH_BINARY)[1] #Modify the 25 threshold: change to lower value makes more sensitive
        ...
        if threshold_L.sum() > 125: #If the images are over a certain threshold difference... ###EDIT THIS TO CHANGE SENSITIVITY (default = 300) - tweak in situ w apparatus
        ...
        if threshold_R.sum() > 125: #If the images are over a certain threshold difference... ###EDIT THIS TO CHANGE SENSITIVITY (default = 300) - tweak in situ w apparatus
        ...
        if L_detection_counter >= 7: #Regardless of whether those two specific images were much different, if the detection counter is high enough:
            R_detection_counter = 0
...
#                print("Motion detected in the left chamber!")
                out = cv2.VideoWriter(str(newpath) + "/Videos/" + get_mp4_NameWithDate("left.mp4"), fourcc, 5, (437, 437)) #Make output stream (to write out content to and to be closed when we're done recording the file)
#                print("Recording in left chamber starting")
            ...
            if timer_started: #and if latency timer already started
                if time.time() - detection_stopped_time >= SECONDS_TO_RECORD_AFTER_DETECTION: #Monitor when latency timer exceeds threshold
...
#                    print("Stopped recording in left chamber")
                ...
                detection_stopped_time = time.time() #Take timestamp that motion counter dipped below threshold

        if R_detection_counter >= 7: #Regardless of whether those two specific images were much different, if the detection counter is high enough:
            L_detection_counter = 0
...
#                print("Motion detected in the right chamber!")
                out = cv2.VideoWriter(str(newpath) + "/Videos/" + get_mp4_NameWithDate("right.mp4"), fourcc, 5, (437, 437)) #Make output stream (to write out content to and to be closed when we're done recording the file)
